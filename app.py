...
df_global = pd.read_csv(os.path.join(base_path, "df_energia_global.csv"))#Cargamos el archivo CSV en un DataFrame de pandas.
df_arg = pd.read_csv(os.path.join(base_path, "dataset_arg.csv"))#Cargamos el archivo CSV con datos de ARgentina en un DataFrame de pandas.
# ------------------- GRUPOS DE PAÍSES -------------------
...
def formato_grande(n):
    if pd.isna(n): #Con esta función de pandas chequeamos si n es un valor faltante, si lo es coloca N/D y sino continua con la funcion
        return "N/D"
    if n >= 1_000_000_000:
       return f"{n/1_000_000_000:.1f}B" #.1f significa mostrar el número con 1 decimal en formato flotante
    if n >= 1_000_000:
...
fig_co2.update_xaxes(showgrid=True, gridcolor="rgba(0,0,0,0.08)") #Definimos q muestre líneasd de la grilla y el color esto es para el eje x y abajo el eje y
...
# ------------------- FIGURA 3: HEATMAP -------------------
    # calculamos la correlacion entre las variables para estudiar qué tan relacionadas están entre sí
df_corr = df_global[[
    "CO2_ton",
    "CO2_per_capita_ton",
    "Energy consumption",
...
df_corr.columns = [ #Cambiamos el nombre a variables para que sea más entendible el heatmap
...
for fig in [fig_arg_renov, fig_arg_demanda, fig_arg_emision]:
...
app.layout = html.Div( #HTML div es un contenedor que agrupa elementos
    ...
    children=[ #el contenido dentro del contenedor
        ...
        html.H1("Dashboard de emisiones y energía", style=TITLE_STYLE), #Título principal 
        html.Div("Análisis global de emisiones y energía", style=SECTION_TITLE_STYLE),

        html.P(
            "Exploración de tendencias globales en emisiones, consumo energético y transición hacia energías renovables.", #Subtítulo
            style=SUBTEXT_STYLE
        ),
        ...
        html.Div( #contenedor del selector agrupa todo el bloque del selector
            ...
            children=[
                ...
                dcc.Slider( #Definimos la creación del slider: componente interactivo para elegir valores
                    ...
                    marks={int(y): str(int(y)) for y in years_disponibles}, #es un diccionario para en donde para cada año seleccionado en el 
                    #slider) lo pasa a texto 
        ...
        html.Div( 
            ...
            children=[ #Acá están las 4 tarjetas KPI
                html.Div([
                    html.Div("Año analizado", style={"fontSize": "13px", "color": MUTED}), #Etiqueta de texto, lo q vemos y el estilo
        ...
        html.Div( #contenedor de los dos gráficos
            ...
            children=[ #Son dos html.Div, cada uno es una card con su gráfico. Una card en este caso es un contenedor visual que agrupa información relacionada.
                ...
                html.Div( #Igual estructura visual del gráfico 2 q el anterior
                    ...
                    children=[
                        ...
                        # Heatmap de correlaciones
                                ...
                                "marginTop": "24px" #agrega espacio arriba
                                 ...
                                 children=[ #Lo que esta dentro del contenedor q es la card. Este primer children dice "el contenedor externo tiene UNA cosa: la card",
                                     #sin este children no podriamos centrar la card
                                html.Div(
                                style={**CARD_STYLE,
                                "width": "100%" #ocupa todo el ancho disponible
                                },
                                children=[#contenido de la card, lo que se ve dentro, este segundo dice el contenido q tiene esa card
...
@app.callback( #Este callback actualiza el gráfico de PBI cada vez que elegís un grupo en el dropdown.
    ...
    Input("dropdown-grupos", "value") #Cuando cambie el valor de dropdown-grupos, q se actualice la figure del grafico con id:"grafico-pbi-dinamico"
)
